...
@Component({
  ...
  styleUrls: ['./java-detail.component.css']
})
export class JavaDetailComponent implements OnInit {
  data = [
    {
      id:1,
      name:'Spring MVC',
  ...
  courseDetail:Object;
  constructor( private s:JavaService, private route: ActivatedRoute) { }

  ngOnInit() {
    this.route.params.subscribe((params: {id: number}) => {
            this.courseDetail = this.data.find(d => d.id == params.id);
      });
  }
